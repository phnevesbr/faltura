-- Corrigir a função get_system_analytics novamente
  ...
  SELECT json_build_object(
    'user_growth', (
      ...
      LEFT JOIN (
        SELECT 
          ...
          COUNT(*) as new_users,
          SUM(COUNT(*)) OVER (ORDER BY DATE(p.created_at)) as total_users
        FROM profiles p
    ...
    'activity_heatmap', (
      SELECT COALESCE(json_object_agg(
        EXTRACT(hour FROM at.created_at)::text,
        activity_count
      ), '{}'::json)
      FROM (
        SELECT 
          EXTRACT(hour FROM at.created_at) as hour,
          COUNT(*) as activity_count
        FROM achievement_tracking at
        WHERE at.created_at >= CURRENT_DATE - (days_back || ' days')::interval
        GROUP BY EXTRACT(hour FROM at.created_at)
      ) hourly_activity
    ),
    'top_courses', (
      SELECT COALESCE(json_agg(
